...
export default function ManagerSoundsPage() {
  ...
  const load = async (category?: string) => {
    setLoading(true);
    try {
      const data = await managerService.getSounds(category);
      const soundsData = typeof data === 'object' && data !== null ? data as SoundResponse : { data: [], categories: ['general'] };
      setSounds(soundsData.data ?? []);
      setCategories(soundsData.categories ?? ['general']);
    } finally {
      setLoading(false);
    }
  };
  ...
  const onUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      await managerService.uploadSound(file, selectedCategory);
  ...
  const onDelete = async (name: string) => {
    await managerService.deleteSound(name, selectedCategory);
    await load(selectedCategory);
  };
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Manage shared audio in storage/app/public/sounds</CardDescription>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2">
              <FolderOpen className="w-4 h-4 text-muted-foreground" />
              <Select value={selectedCategory} onValueChange={(value: string | null) => setSelectedCategory(value || 'general')}>
