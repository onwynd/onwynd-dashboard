...
export function StatsCards() {
  const stats = usePartnerStore((state) => state.stats);

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
      {stats.map((stat) => {
        const key = (stat.iconName?.toLowerCase?.() as IconKey) || "users";
        const Icon = iconMap[key] || Users;
        const SubtitleIcon = iconMap[(stat.iconName?.toLowerCase?.() as IconKey) || "file"] || File;
