...
export function DealsTable() {
  ...
  const stageFilter = useSalesStore((state) => state.stageFilter);
  const ownerFilter = useSalesStore((state) => state.ownerFilter);
  const valueFilter = useSalesStore((state) => state.valueFilter);
  const setSearchQuery = useSalesStore((state) => state.setSearchQuery);
  const setStageFilter = useSalesStore((state) => state.setStageFilter);
  const setOwnerFilter = useSalesStore((state) => state.setOwnerFilter);
  const setValueFilter = useSalesStore((state) => state.setValueFilter);
  const clearFilters = useSalesStore((state) => state.clearFilters);
  ...
  }, [deals, searchQuery, stageFilter, ownerFilter, valueFilter]);
  ...
  const handleExport = () => {
    const headers = ["Deal Name", "Client", "Amount", "Stage", "Owner", "Expected Close"];
    ...
    downloadCSV("deals.csv", headers, rows);
  };
    ...
    <div className="rounded-xl border bg-card">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-3 sm:px-6 sm:py-3.5">
        <div className="flex items-center gap-2 sm:gap-2.5 flex-1">
          <Button variant="outline" size="icon" className="size-7 sm:size-8 shrink-0" onClick={handleExport} title="Export CSV">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-3 sm:px-6 py-3 border-t">
        <div className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground">
          ...
          <Select
            ...
            onValueChange={(value: string | null) => setPageSize(Number(value ?? "10"))}
          >
