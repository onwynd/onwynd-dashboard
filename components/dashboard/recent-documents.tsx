...
export function RecentDocuments() {
  const { documents, documentsError } = useDashboardStore();
    ...
    <div className="relative overflow-hidden rounded-xl border border-border bg-card max-h-[400px] flex flex-col">
      ...
      <div className="px-[14px] pb-4 overflow-y-auto flex-1">
        {documentsError ? (
          ...
          <p className="py-6 text-center text-sm text-muted-foreground">No documents found.</p>
        ) : null}
        <div className="space-y-[8px]">
          {!documentsError && documents.map((doc) => {
            const Icon = iconMap[doc.type as keyof typeof iconMap] || FileText;
            return (
              <div
                key={doc.id}
