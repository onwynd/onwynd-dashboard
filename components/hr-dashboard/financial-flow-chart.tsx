...
// Remove getDataForPeriod helper as we use store data
...
function CustomTooltip({ active, payload, label }: CustomTooltipProps) {
  ...
  const moneyInChange = Number((moneyInData?.payload as Record<string, unknown>)?.moneyInChange ?? 0);
  const moneyOutChange = Number((moneyOutData?.payload as Record<string, unknown>)?.moneyOutChange ?? 0);
